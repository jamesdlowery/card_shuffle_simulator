...
Simulates realistic card shuffling (Gilbert-Shannon-Reeds or simple riffle)
on one or more decks with optional jokers and selectable suits.
Supports resume from full or abridged log files, progress reporting,
elapsed-time tracking, and dual logging (verbose + abridged summary).
"""
...
def card_to_str(card_id, num_decks, deck_size, cards_per_suit_set):
    """Convert internal card ID to human-readable string.
    cards_per_suit_set is required so joker detection works correctly
    when fewer than 4 suits are selected.
    """
    ...
    if local_id < cards_per_suit_set:
        suit_id = local_id // 13
        rank_id = local_id % 13
        return prefix + RANKS[rank_id] + SUITS[suit_id]
    else:
        # Jokers are always appended at cards_per_suit_set and +1
        return prefix + ('J1' if local_id % 2 == 0 else 'J2')

def compute_deck_size(selected_suits, include_jokers):
    """Return total cards per deck based on suits and jokers."""
    return len(selected_suits) * 13 + (2 if include_jokers == 'y' else 0)

def build_deck(num_decks, include_jokers, initial_order, selected_suits):
    """Build the deck as a list of integer IDs.
    Returns (deck, cards_per_suit_set).
    """
    ...
    for d in range(num_decks):
        ...
        if include_jokers == 'y':
            deck.append(base + cards_per_suit_set)
            deck.append(base + cards_per_suit_set + 1)
        base += cards_per_deck

    if initial_order == 'r':
        random.shuffle(deck)

    return deck, cards_per_suit_set

def gsr_shuffle(deck):
    """Gilbert-Shannon-Reeds riffle shuffle (O(n) via deque)."""
    n = len(deck)
    if n <= 1:
        return deck[:]
...
def simple_riffle(deck):
    """Simple deterministic-cut riffle with random starting side."""
    n = len(deck)
    k = n // 2
    left = deck[:k]
    right = deck[k:]
...
def get_resources():
    """Return current peak memory usage in MB (Windows-safe)."""
    try:
        mem_kb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
        mem_mb = mem_kb / 1024.0
        if sys.platform == 'darwin':
            mem_mb /= 1024
        return f"Peak memory: {mem_mb:.1f} MB"
    except Exception:
        return "Peak memory: N/A (Windows)"

def parse_elapsed_from_line(val):
    """Extract elapsed time from a log line containing 'elapsed: X.XXXX'."""
    if not val:
        return 0.0
    try:
        if 'elapsed:' in val:
            after = val.split('elapsed:', 1)[1].strip()
...
def overwrite_abridged_summary(abridged_file, params, original, deck, count, total_elapsed, num_decks, deck_size):
    """Write (overwrite) the abridged summary log with latest state."""
...
def main():
    print(f"Card Shuffle Simulator v{__version__} ({__date__})")
    print("-" * 50)
    ...
    verbose_log_handle = None
    params = {}
    cards_per_suit_set = 52  # safe default, overwritten in both paths
    ...
    if load:
        ...
        deck_size = compute_deck_size(selected_suits, params.get('Include jokers', 'n'))
        cards_per_suit_set = len(selected_suits) * 13
        ...
        if completed:
            ...
            print(' '.join(card_to_str(c, num_decks, deck_size, cards_per_suit_set) for c in deck))
            return

        print("Beginning order: " + ' '.join(card_to_str(c, num_decks, deck_size, cards_per_suit_set) for c in original))
        if count > 0:
        ...
        if is_abridged:
            ...
            # Resume from full verbose log → continue appending to it
            verbose_log_file = filename
            abridged_log_file = filename.replace('.log', '_abridged.log')
            log_verbose = True
            try:
                verbose_log_handle = open(verbose_log_file, 'a')
            except Exception as e:
                print(f"Warning: could not open verbose log for append: {e}")
                verbose_log_handle = None
                log_verbose = False
        ...
        # Fresh run – robust input collection
        while True:
        ...
        deck_size = compute_deck_size(selected_suits, include_jokers)
        original, cards_per_suit_set = build_deck(num_decks, include_jokers, initial_order_str.lower(), selected_suits)
        deck = original[:]
        count = 0
        ...
        if log_verbose:
            ...
            verbose_log_handle = open(verbose_log_file, 'w')
            for k, v in params.items():
                verbose_log_handle.write(f"{k}: {v}\n")
            verbose_log_handle.write(f"Initial: {' '.join(map(str, original))}\n")
        ...
        if loop_until:
            while True:
                ...
                if verbose_log_handle:
                    verbose_log_handle.write(f"Shuffle {count}: {' '.join(map(str, deck))} elapsed: {total_elapsed:.4f}\n")
