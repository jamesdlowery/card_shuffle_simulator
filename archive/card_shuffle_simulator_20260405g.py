...
def run_shuffles(deck, shuffle_func, log_verbose, verbose_log_file, abridged_log_file, params, original,
                 progress_every, num_decks, deck_size, session_start, time_offset, target_shuffles=None):
    """Unified shuffle loop body - used by both loop-until-original and fixed-count modes"""
    count = 0
    while True:
        ...
        if target_shuffles is not None and count >= target_shuffles:
            break
        if deck == original:
            print(f"Alert: Deck has returned to exact original order after {count} shuffles!")
            break

        # Update abridged log and show progress only when we actually print to screen
...
def main():
    ...
    if load:
        # ... (resume logic unchanged for brevity - same as previous version)
    ...
    final_deck, final_count, final_elapsed = run_shuffles(
        ...
        target_shuffles=None if loop_until else num_shuffles_param
    )
