...
def get_resources():
    """Returns current peak memory usage in MB (Windows-safe)"""
    try:
...
def parse_elapsed_from_line(val):
    """Robustly extract elapsed time from log line containing 'elapsed: X.XXXX'"""
    if not val:
        return 0.0
    try:
        if 'elapsed:' in val:
...
def run_shuffles(deck, shuffle_func, log_verbose, verbose_log_file, abridged_log_file, params, original,
                 progress_every, num_decks, deck_size, session_start, time_offset, is_loop_until):
    """Unified shuffle execution - handles both loop-until-original and fixed-count modes"""
    count = 0
    target_shuffles = None if is_loop_until else params.get('How many shuffles', 1000000)
    ...
    while True:
        ...
        # Stopping conditions
        if not is_loop_until and count >= target_shuffles:
            break
        if deck == original:
            print(f"Alert: Deck has returned to exact original order after {count} shuffles!")
            break

        # Update abridged and show progress only when printing to screen
        if progress_every > 0 and count % progress_every == 0:
            # Skip final progress print when not looping (we'll do final update at end)
            if not is_loop_until and count == target_shuffles:
                pass
            else:
    ...
    # Always do final abridged update
    total_elapsed = time_offset + (time.time() - session_start)
    overwrite_abridged_summary(abridged_log_file, params, original, deck, count, total_elapsed, num_decks, deck_size)

    return deck, count, total_elapsed
...
def main():
    ...
    # Unified shuffle execution
    final_deck, final_count, final_elapsed = run_shuffles(
        deck=deck,
        shuffle_func=shuffle_func,
        log_verbose=log_verbose,
        verbose_log_file=verbose_log_file,
        abridged_log_file=abridged_log_file,
        params=params,
        original=original,
        ...
        is_loop_until=loop_until
    )

    print(f"After {final_count} shuffles | Total time: {format_time(final_elapsed)}")
    print(' '.join(card_to_str(c, num_decks, deck_size) for c in final_deck))
