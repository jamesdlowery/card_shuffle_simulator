...
def write_abridged_summary(abridged_file, params, original, deck, count, total_elapsed, num_decks, deck_size, is_resume=False):
    mode = 'a' if is_resume else 'w'
    with open(abridged_file, mode) as f:
        if not is_resume:
        ...
        f.write(f"Total elapsed time: {total_elapsed:.4f} seconds ({format_time(total_elapsed)})\n")
...
def main():
    ...
    if load:
        ...
        if not is_abridged:
            ...
            # Parse as abridged log
            in_update = False
            for line in lines:
                line = line.strip()
                if line.startswith('Number of decks:'):
                    ...
                    try:
                        ...
                        continue
                elif line.startswith('--- Update at shuffle'):
                    num_str = line.split('shuffle ', 1)[1].rstrip(' ---')
                    if num_str.isdigit():
                        last_count = int(num_str)
                        in_update = True
                elif in_update and line.startswith('Total elapsed time:'):
                    val = line.split(':', 1)[1].strip()
                    secs_str = val.split(' seconds')[0]
                    last_elapsed = float(secs_str)
                elif in_update and line.startswith('Current deck (raw):'):
                    val = line.split(':', 1)[1].strip()
        ...
        # Ensure abridged exists when resuming from full
        if not os.path.exists(abridged_log_file):
            write_abridged_summary(abridged_log_file, params, original, deck, count, time_offset, num_decks, deck_size, is_resume=False)
        else:
            write_abridged_summary(abridged_log_file, params, original, deck, count, time_offset, num_decks, deck_size, is_resume=True)
        ...
        write_abridged_summary(abridged_log_file, params, original, deck, count, 0.0, num_decks, deck_size, is_resume=False)
        ...
        print("Beginning order: " + ' '.join(card_to_str(c, num_decks, deck_size) for c in deck))

    # Start timer after all inputs/setup — this is the session timer
    ...
    if loop_until:
        while True:
            ...
            if abridged_log_file:
                write_abridged_summary(abridged_log_file, params, original, deck, count, total_elapsed, num_decks, deck_size, is_resume=True)
