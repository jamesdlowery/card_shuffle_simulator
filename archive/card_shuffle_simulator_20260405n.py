...
def card_to_str(card_id, num_decks, deck_size, cards_per_suit_set):
    ...
    cards_per_suit_set is needed to correctly identify joker boundary."""
...
def build_deck(num_decks, include_jokers, initial_order, selected_suits):
    ...
    return deck, cards_per_suit_set   # return both deck and suit set size
...
def main():
    ...
    if load:
        ...
        if completed:
            ...
            print(' '.join(card_to_str(c, num_decks, deck_size, len(selected_suits)*13) for c in deck))
            return

        print("Beginning order: " + ' '.join(card_to_str(c, num_decks, deck_size, len(selected_suits)*13) for c in original))
        if count > 0:
