...
def write_abridged_summary(log_file, params, original, deck, count, total_elapsed, num_decks, deck_size, is_resume=False):
    mode = 'a' if is_resume else 'w'
    with open(log_file, mode) as f:
        if not is_resume:
            f.write("=== Card Shuffle Simulator - Abridged Summary ===\n")
            f.write(f"Started: {datetime.datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}\n")
            f.write(f"Number of decks: {params.get('num_decks', params.get('Number of decks', 1))}\n")
            f.write(f"Include jokers: {params.get('include_jokers', params.get('Include jokers', 'n'))}\n")
            f.write(f"Initial order: {params.get('initial_order', params.get('Initial order', 'o'))}\n")
            f.write(f"Shuffle method: {params.get('method', params.get('Shuffle method', 'g'))}\n")
            f.write(f"Loop until original: {params.get('loop_until', params.get('Loop until original', 'n'))}\n")
            f.write(f"Target shuffles: {params.get('num_shuffles_param', params.get('How many shuffles', 1000000))}\n")
            f.write(f"Initial deck (raw): {' '.join(map(str, original))}\n")
            f.write("\n")

        f.write(f"--- Update at shuffle {count} ---\n")
        f.write(f"Timestamp: {datetime.datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}\n")
        f.write(f"Total elapsed time: {total_elapsed:.2f} seconds ({format_time(total_elapsed)})\n")
        f.write(f"Current deck (raw): {' '.join(map(str, deck))}\n")
        f.write("\n")
...
def main():
    ...
    if load:
        ...
        params = {
            ...
            'initial_order': initial_order_str.lower(),
            'method': method,
            'loop_until': loop_str.lower(),
            'num_shuffles_param': num_shuffles_param
        }
        ...
        # Always create abridged log when starting fresh
        abridged_log_file = f"card_shuffle_simulator_{timestamp}_abridged.log"
        write_abridged_summary(
            abridged_log_file, params, original, deck, count, 0.0,
            num_decks, deck_size, is_resume=False
        )

        print("Beginning order: " + ' '.join(card_to_str(c, num_decks, deck_size) for c in original))

    # Timer starts after all inputs and setup
    session_start = time.time()

    shuffle_func = gsr_shuffle if method == 'g' else simple_riffle
    deck_size = 54 if 'include_jokers' in params and params['include_jokers'] == 'y' else 52
    num_decks = params.get('num_decks', 1)
    ...
    if loop_until:
        while True:
            ...
            # Always update abridged log
            write_abridged_summary(
                abridged_log_file, params, original, deck, count, total_elapsed,
                num_decks, deck_size, is_resume=load
            )
