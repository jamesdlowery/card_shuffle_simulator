...
def write_abridged_summary(abridged_file, params, original, deck, count, total_elapsed, num_decks, deck_size, is_first=False):
    ...
    with open(abridged_file, mode) as f:
        ...
        f.write(f"Update after shuffle {count}\n")
...
def main():
    ...
    if load:
        ...
        if not is_abridged:
            # Full verbose log parsing (same as before)
            for line in lines:
                line = line.strip()
            ...
            for line in lines:
                ...
                elif line.startswith('Initial deck (raw):'):
                    val = line.split(':', 1)[1].strip()
                    try:
                        original = [int(x) for x in val.split()]
                    except:
                        continue
                elif line.startswith('Update after shuffle'):
                    num_str = line.split('shuffle ', 1)[1].strip()
                    if num_str.isdigit():
                        last_count = int(num_str)
                        current_update = True
                elif current_update and line.startswith('Elapsed time:'):
                    val = line.split(':', 1)[1].strip()
                    secs_part = val.split(' s')[0]
                    last_elapsed = float(secs_part)
                elif current_update and line.startswith('Deck (raw):'):
                    val = line.split(':', 1)[1].strip()
        ...
        # Write initial state only once
        write_abridged_summary(abridged_log_file, params, original, deck, 0, 0.0, num_decks, deck_size, is_first=True)
    ...
    if loop_until:
        while True:
            ...
            # Update abridged only when we would show progress (or at end)
            if progress_every > 0 and count % progress_every == 0:
                write_abridged_summary(abridged_log_file, params, original, deck, count, total_elapsed, num_decks, deck_size, is_first=False)

            if deck == original:
                print(f"Alert: Deck has returned to exact original order after {count} shuffles!")
                # Final update even if not on progress interval
                write_abridged_summary(abridged_log_file, params, original, deck, count, total_elapsed, num_decks, deck_size, is_first=False)
                break
        ...
        while count < target_shuffles:
            ...
            # Update abridged only on visible progress points
            if progress_every > 0 and count % progress_every == 0:
                write_abridged_summary(abridged_log_file, params, original, deck, count, total_elapsed, num_decks, deck_size, is_first=False)
    ...
    # Always do final write if we haven't already (especially when progress_every == 0)
