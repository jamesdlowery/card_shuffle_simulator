...
def overwrite_abridged_summary(abridged_file, params, original, deck, count, total_elapsed, num_decks, deck_size):
    ...
    with open(abridged_file, 'w') as f:
        ...
        f.write(f"  Progress every: {params.get('Show progress every how many shuffles', 0)} shuffles\n")
        f.write(f"  Target shuffles: {params.get('How many shuffles', 1000000)}\n\n")
        
        f.write(f"Initial deck (raw):\n  {' '.join(map(str, original))}\n\n")
...
def main():
    ...
    if load:
        ...
        if not is_abridged:
            ...
            # Abridged parsing – look for the last visible deck block
            current_count = 0
        ...
        if is_abridged:
            ...
            abridged_log_file = filename.replace('.log', '_abridged.log') if '.log' in filename else filename + '_abridged.log'
            log_verbose = True

        # Overwrite abridged with current resuming state
    ...
    if loop_until:
        while True:
            ...
            # Overwrite abridged only when we print to screen
        ...
        while count < target_shuffles:
            ...
            # Overwrite abridged only when printing to screen
    ...
    # Always overwrite with final state (especially important when progress_every == 0)
